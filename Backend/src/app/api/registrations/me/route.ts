...
export const GET = withErrorHandler(async (request: NextRequest) => {
  ...
  // This shows both pending and historical registrations (approved/rejected)
  ...
  // Filter out password_hash from response
  const safeRegistrations = (registrations || []).map(reg => {
    const { password_hash, ...safe } = reg;
    return safe;
  });

  return successResponse(safeRegistrations || []);
});

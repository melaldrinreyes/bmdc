...
export const PATCH = withErrorHandler(async (request: NextRequest, { params }: { params: { id: string } | Promise<{ id: string }> }) => {
  ...
  if (action === 'approve') {
    const result = await registrationService.approveRegistration(resolvedParams.id, authResult.user.userId);
    ...
    await registrationService.rejectRegistration(resolvedParams.id, authResult.user.userId, rejection_reason);
